...
def get_custom_film(code):
    conn = sqlite3.connect("filmix.db")
    c = conn.cursor()
    row = c.execute("SELECT code, title, year, description, poster, watch_url FROM custom_films WHERE code=?", (str(code),)).fetchone()
    conn.close()
    if row:
        return {"code": row[0], "title": row[1], "year": row[2], "description": row[3], "poster": row[4], "watch_url": row[5]}
    return None
